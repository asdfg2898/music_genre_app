...
def load_models():
    ...
    try:
        # XGBoost 모델 로드[6][8]
        model = xgb.Booster()
        model.load_model('my_xgboost_model.json')
...
def predict_genre(audio_file, model, scaler, label_encoder, feature_cols):
    ...
    # 장르명 변환
    genre_name = label_encoder.inverse_transform([prediction])[0]
    
    return genre_name, prediction_proba[0] if len(prediction_proba.shape) > 1 else prediction_proba
